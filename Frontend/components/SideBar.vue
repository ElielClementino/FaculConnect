<template>
    <v-navigation-drawer
        color="#010101"
        permanent
        expand-on-hover
        app
    >
        <v-list>
            <v-list-item>
                <v-list-item-icon class="d-flex justify-center">
                    <p class="top-title"><svg style="color: 6BE7E7;" xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24">
		    <path fill="currentColor" d="M21 18.5h-6.18A3 3 0 0 0 13 16.68V13.5h3.17a4.33 4.33 0 0 0 1.3-8.5A6 6 0 0 0 6.06 6.63A3.5 3.5 0 0 0 7 13.5h4v3.18a3 3 0 0 0-1.82 1.82H3a1 1 0 0 0 0 2h6.18a3 3 0 0 0 5.64 0H21a1 1 0 0 0 0-2m-14-7a1.5 1.5 0 0 1 0-3a1 1 0 0 0 1-1a4 4 0 0 1 7.79-1.29a1 1 0 0 0 .78.67a2.31 2.31 0 0 1 1.93 2.29a2.34 2.34 0 0 1-2.33 2.33Zm5 9a1 1 0 1 1 1-1a1 1 0 0 1-1 1" />
	    </svg></p>
                </v-list-item-icon>
                <v-list-item-title class="title text-style" style="margin-top:7%;"><span style="font-weight: 100;">Facul</span>Connect</p></v-list-item-title>
            </v-list-item>
        </v-list>
        <div class="pt-8">
        <v-list class="mt-8">
            <v-list-item class="pb-8" link>
                <v-list-item-icon>
                    <v-icon x-large>
                        mdi-home-outline
                    </v-icon>
                    <v-list-item-title class="title text-style ml-4 mt-2">Página Inicial</v-list-item-title>
                </v-list-item-icon>
            </v-list-item>
            <v-list-item class="pt-6" link>
                <v-list-item-icon>
                    <v-icon @click="redirectToCourses" x-large>
                        mdi-bookshelf
                    </v-icon>
                    <v-list-item-title @click="redirectToCourses" lass="title text-style ml-4 mt-2">Cursos</v-list-item-title>
            </v-list-item-icon>
            </v-list-item>
        </v-list>
        </div>
    </v-navigation-drawer>
</template>

<script>
export default {
    methods: {
        redirectToCourses() {
            this.$router.push('/CoursesPage');
        }
    }
}
</script>
<style scoped>

.top-title {
  position: absolute;

}

.text-style {
    font-family: Poppins, sans-serif;
    font-weight: 900;
}
</style>
